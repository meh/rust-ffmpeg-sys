...
version_minor=version_minor));
    ...
    write!(File::create(out_dir.join("check.c")).expect("Failed to create file"),
           r#"
            #include <stdio.h>
            {includes_code}
    ...
    for &(_, feature, var) in infos {
        ...
        if &stdout[pos + 1..pos + 2] == "1" {
            println!(r#"cargo:rustc-cfg=feature="{}_is_defined""#,
                     var.to_lowercase());
...
fn main() {
    ...
    check_features(include_paths.clone(),
                   &vec![("libavutil/avutil.h", None, "FF_API_OLD_AVOPTIONS"),
